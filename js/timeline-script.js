...
(function () {
  ...
  if (timeline) {
    timeline.addEventListener("wheel", scrollH, { passive: false }); //browsers use wheel event
  }
})();
  ...
  if (event.key.toLowerCase() === "p") {
    ...
    if (rotatingElement) {
      if (rotatingElement.classList.contains("rotate")) {
        ...
        const currentTransform = computedStyle.transform;
        rotatingElement.style.transform = currentTransform;
        rotatingElement.classList.add("rotate"); // Add back rotation
      }
    }
  }
});
...
document.addEventListener("keydown", (event) => {
  if (event.key.toLowerCase() === "t") {
    if (isBehind) {
      // Set blur and rotating image z-index to 10 and 20 respectively (to move them in front)
